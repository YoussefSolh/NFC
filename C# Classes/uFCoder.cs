...
namespace uFR
{
    ...
    public static class uFCoder
    {
        ...
        public static extern DL_STATUS GetReaderSerialDescription(ref byte pSerialDescription);
        ...
        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTD_MRZSubjacentCheck")]
        public static extern DL_STATUS MRTD_MRZSubjacentCheck(char[] mrz);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTD_MRZCheck")]
        public static extern DL_STATUS MRTD_MRZCheck(char[] mrz, int length);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTD_MRZDataToMRZProtoKey")]
        public static extern DL_STATUS MRTD_MRZDataToMRZProtoKey(string doc_number, string date_of_birth, string date_of_expiry, byte[] mrz_proto_key);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDAppSelectAndAuthenticateBac")]
        public static extern DL_STATUS MRTDAppSelectAndAuthenticateBac([In] byte[] mrz_proto_key, [Out] byte[] ksenc, [Out] byte[] ksmac, ref UInt64 send_sequence_cnt);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDFileReadBacToHeap")]
        public static extern DL_STATUS MRTDFileReadBacToHeap([In] byte[] file_index, out IntPtr output, out UInt32 output_length, [In] byte[] ksenc, [In] byte[] ksmac, ref UInt64 send_sequence_cnt);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDParseDG1ToHeap")]
        private static extern DL_STATUS MRTDParseDG1ToHeap(out IntPtr sbuffer, StringBuilder newln, IntPtr dg1, UInt32 dg1_len);



        public static DL_STATUS MRTDParseDG1ToHeap(out String out_str, String newln, IntPtr dg1, UInt32 dg1_len)
        {
            DL_STATUS status;
            IntPtr sbuffer;
            StringBuilder ptr_newln = new StringBuilder(newln);

            out_str = "";
            status = MRTDParseDG1ToHeap(out sbuffer, ptr_newln, dg1, dg1_len);
            if (status == DL_STATUS.UFR_OK)
                out_str = Marshal.PtrToStringAnsi(sbuffer);
            DLFree(sbuffer);
            return status;
        }

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDGetImageFromDG2")]
        public static extern DL_STATUS MRTDGetImageFromDG2(IntPtr dg2, UInt32 dg2_size, out IntPtr image, out UInt32 image_size, out UInt32 image_type);

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDGetImageFromDG2ToFile")]
        private static extern DL_STATUS MRTDGetImageFromDG2ToFile(IntPtr dg2, UInt32 dg2_size, StringBuilder file_name);

        public static DL_STATUS MRTDGetImageFromDG2ToFile(IntPtr dg2, UInt32 dg2_size, String file_name_without_extension)
        {
            StringBuilder file_name = new StringBuilder(file_name_without_extension);
            return MRTDGetImageFromDG2ToFile(dg2, dg2_size, file_name);
        }

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "MRTDValidate")]
        public static extern DL_STATUS MRTDValidate(StringBuilder cert_storage_folder, out IntPtr out_str, StringBuilder newln, UInt32 verbose_level, [In] byte[] ksenc, [In] byte[] ksmac, ref UInt64 send_sequence_cnt);

        public static DL_STATUS MRTDValidate(String cert_storage_folder, out String out_str, String newln, UInt32 verbose_level, [In] byte[] ksenc, [In] byte[] ksmac, ref UInt64 send_sequence_cnt)
        {
            DL_STATUS status;
            IntPtr sbuffer;
            StringBuilder ptr_csca_folder = new StringBuilder(cert_storage_folder);
            StringBuilder ptr_newln = new StringBuilder(newln);

            out_str = "";
            status = MRTDValidate(ptr_csca_folder, out sbuffer, ptr_newln, verbose_level, ksenc, ksmac, ref send_sequence_cnt);
            out_str = Marshal.PtrToStringAnsi(sbuffer);
            DLFree(sbuffer);
            return status;
        }

        [DllImport(DLL_NAME, CallingConvention = CallingConvention.StdCall, EntryPoint = "DLFree")]
        public static extern void DLFree(IntPtr ptr);
